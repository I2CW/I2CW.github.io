...
function printpdf() { 
	...
	var inputIntro = tinyMCE.get('intro').getContent({ format: 'text' });
	...
	for (i = 0; i < xbody; i++) {
			...
			if (i < (xbody-1)) {
				inputbody = inputbody + ' ' + tinyMCE.get(paragraph).getContent({ format: 'text' }) + "\n \t \t \t \t \t \t ";
			} else {
				inputbody = inputbody + ' ' + tinyMCE.get(paragraph).getContent({ format: 'text' });    
			}

		} 
	var inputconclusion= tinyMCE.get('conclusion').getContent({ format: 'text' })
